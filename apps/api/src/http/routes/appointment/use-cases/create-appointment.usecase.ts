...
export const createAppointmentUseCase = async (
  ...
  const formattedDate = addHours(parseISO(date), 3)
  // convert data para hora
  const appointmentDate = startOfHour(formattedDate)
  // Separa a hora
  const appointmentHour = appointmentDate.getHours()
  // Verificar se o horario é antes do horario de inicio
  if (
    appointmentHour < business.startWorkHour ||
    appointmentHour >= business.endWorkHour
  ) {
  ...
  if (appointment) {
    throw new ConflictError(
      'Já existe um agendamento neste horário, tente outro horário',
    )
  }
